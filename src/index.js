...
function filterValidRules(rules, config) {
    ...
    return Object.fromEntries(
        Object.entries(rules).filter(([_, action]) => validActions.includes(action))
    );
}

function updateProxyGroup(config) {
    const proxies = config.proxies || [];
    const baseCostProxies = proxies.filter(proxy => {
...
async function clashConverter(request) {
	...
	async fetch(request) {
        ...
        switch (route) {
            case 'clash_converter':
                return await clashConverter(request);
        }
        return new Response('base api not found', {
            status: 404
        });
	},
  };
